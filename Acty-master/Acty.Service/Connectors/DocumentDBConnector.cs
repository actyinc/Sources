...
{
    internal static class DocumentDBConnector
    {
        ...
        private static string UsersCollectionId
        {
            get
            {
                if (_usersCollectionId == null)
                {
                    _usersCollectionId = ConfigurationManager.AppSettings["UsersCollectionId"];
                }

                return _usersCollectionId;
            }
        }

        private static string CampaignsCollectionId
        {
            get
            {
                if (_campaignsCollectionId == null)
                {
                    _campaignsCollectionId = ConfigurationManager.AppSettings["CampaignsCollectionId"];
                }

                return _campaignsCollectionId;
            }
        }

        private static string CommentsCollectionId
        {
            get
            {
                if (_commentsCollectionId == null)
                {
                    _commentsCollectionId = ConfigurationManager.AppSettings["CommentsCollectionId"];
                }

                return _commentsCollectionId;
            }
        }

        private static string EventsCollectionId
        {
            get
            {
                if (_eventsCollectionId == null)
                {
                    _eventsCollectionId = ConfigurationManager.AppSettings["EventsCollectionId"];
                }

                return _eventsCollectionId;
            }
        }

        private static string UsersStorageUrl
        {
            get
            {
                if (_usersStorageUrl == null)
                {
                    _usersStorageUrl = ConfigurationManager.AppSettings["UsersStorageUrl"];
                }

                return _usersStorageUrl;
            }
        }

        private static string CampaignsStorageUrl
        {
            get
            {
                if (_campaignsStorageUrl == null)
                {
                    _campaignsStorageUrl = ConfigurationManager.AppSettings["CampaignsStorageUrl"];
                }

                return _campaignsStorageUrl;
            }
        }
        ...
        private static DocumentCollection UsersCollection
        {
            get
            {
                ...
                {
                    _usersCollection = ReadOrCreateCollection(Database.SelfLink, UsersCollectionId);
                }
        ...
        private static DocumentCollection CampaignCollection
        {
            get
            {
                ...
                {
                    _campaignsCollection = ReadOrCreateCollection(Database.SelfLink, CampaignsCollectionId);
                }
        ...
        private static DocumentCollection CommentsCollection
        {
            get
            {
                ...
                {
                    _commentsCollection = ReadOrCreateCollection(Database.SelfLink, CommentsCollectionId);
                }
        ...
        private static DocumentCollection EventsCollection
        {
            get
            {
                ...
                {
                    _eventsCollection = ReadOrCreateCollection(Database.SelfLink, EventsCollectionId);
                }
        ...
        {
            var userNames = Client.CreateDocumentQuery<DBUser>(UsersCollection.DocumentsLink).Where(user => user.Email == userName).AsEnumerable();
        ...
        public static async Task<bool> UpdateUserProfilePicture(UserPicture userPic)
        {
            ...
            var userDoc = Client.CreateDocumentQuery<DBUser>(UsersCollection.DocumentsLink, querySpec).AsEnumerable().FirstOrDefault();
        ...
        public static async Task<bool> UpdateUserPreference(User user, bool isAdd)
        {
            ...
            DBUser doc = Client.CreateDocumentQuery<DBUser>(UsersCollection.DocumentsLink, querySpec).AsEnumerable().FirstOrDefault();
        ...
        public static DBUser GetUser(string userId)
        {
            ...
            if (!string.IsNullOrEmpty(blobLink) && !string.IsNullOrWhiteSpace(blobLink))
                userDoc.UserProfilePictureBlob = UsersStorageUrl + blobLink;
        ...
        public static IEnumerable<DBCampaign> GetCampaignsForUser(string userId)
        {
            ...
            foreach (var campaign in campaigns)
            {
                ...
                if (!string.IsNullOrEmpty(blobLink) && !string.IsNullOrWhiteSpace(blobLink))
                    campaign.StoryMediaResourceBlob = CampaignsStorageUrl + blobLink;
            }

            return campaigns;
        }

        public static IEnumerable<DBCampaign> GetTopFeedsForUser(string userName)
        {
        ...
        internal static async Task<bool> DeleteUser(string strUserName)
        {
            ...
            var doc = Client.CreateDocumentQuery<DBUser>(UsersCollection.DocumentsLink, querySpec).AsEnumerable().FirstOrDefault();
